...
function CastTargetObstructionRayNew(source, target, hitpoints, rayFlags, weaponType, targetId, weaponId)
	...
	repeat
		...
		elseif not (GetRayHitTeamId() == teamId and (GetRayHitDoor() or hitSaveName == weaponType) or hitSaveName == "backbracing") then -- ignore friendly doors
			if hitSaveName ~= "backbracing" or (weaponType == "buzzsaw" or weaponType == "howitzer") then -- ignore backbracing unless buzz or howie
				if (GetRayHitSideId()%MAX_SIDES == teamId) then return data.RAY_HIT_OBSTRUCTED, 0 end -- projectile path collides with friendly entity			
				...
				if data.StructureHPList[hitSaveName] ~= nil then
					...
					if (nodeIdA > 0 and nodeIdB > 0) then
						...
						if data.OpenDoors[nodeIdA .. " " .. nodeIdB] ~= true then SpawnLine(NodePosition(nodeIdA), NodePosition(nodeIdB), Red(), 0.2) --HighlightLink(nodeIdA, nodeIdB, highlight)  then
